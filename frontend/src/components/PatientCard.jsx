      ...
      <div className="p-5">
          ...
          <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-800/60">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-end">
                ...
                <span className="text-sm font-semibold text-slate-200">{preWeight ?? '--'} <small className="font-normal text-slate-500">kg</small></span>
              </div>
              <div className="flex justify-between items-end">
                <span className="text-[11px] text-slate-400">BP</span>
                <span className="text-sm font-mono font-semibold text-slate-200 tracking-tighter">
            ...
            {postWeight != null ? (
              <div className="space-y-2">
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-sm font-semibold text-indigo-100">{postWeight} <small className="font-normal text-indigo-300/50">kg</small></span>
                </div>
                <div className="flex justify-between items-end">
                  <span className="text-[11px] text-indigo-300/60">BP</span>
                  <span className="text-sm font-mono font-semibold text-indigo-100 tracking-tighter">
        ...
        {hasAnomalies && (
          <div className="space-y-2 mb-5">
            <h4 className="text-[10px] font-black text-rose-500 uppercase tracking-widest">Clinical Alerts</h4>
            <div className="flex flex-wrap gap-1.5">
              {anomalies.map((code, i) => (
                <span 
                  key={code + i} 
                  className={`text-[10px] font-bold px-2 py-1 rounded border leading-none ${anomalyConfig[code]?.bg || "bg-slate-800 text-slate-400 border-slate-700"}`}
                >
        ...
        <div className="flex items-center gap-2 pt-4 border-t border-slate-800/50">
          {status === "scheduled" && (
            <button 
              className="btn btn-primary btn-sm w-full bg-indigo-600 hover:bg-indigo-500 border-none shadow-lg shadow-indigo-500/20" 
              onClick={onStart}
            >
              Start Session
            </button>
          ...
          {status === "in-progress" && (
            <>
              <button 
                className="btn btn-ghost btn-sm text-slate-400 hover:text-white hover:bg-slate-800" 
                onClick={onEditNotes}
              >
                Add Notes
              </button>
              <button 
                className="btn btn-success btn-sm flex-1 text-white bg-emerald-600 hover:bg-emerald-500 border-none shadow-lg shadow-emerald-500/20" 
                onClick={onComplete}
              >
                Complete
              </button>
          ...
          {status === "completed" && (
            <button 
              className="btn btn-outline btn-sm w-full border-slate-700 text-slate-300 hover:bg-slate-800 hover:border-slate-600" 
              onClick={onView}
            >
              View Full Summary
            </button>
